...
2. "characters" - 角色信息数组，每个包含：
   - name: 角色名称
   - description: 角色外貌描述（基础描述，不含具体服装）
   - costumes: 服装变体数组（可选）。提取规则如下：
      a. 如果剧本中同一角色出现了多套不同的服装/造型/装扮，请提取为服装变体。
      b. 【年龄×服装交叉】如果同一角色在剧本中跨越不同年龄阶段（如少年期、青年期、中年期），则每个年龄段的每套服装都必须作为独立变体。label格式为"年龄段·服装名"，例如"18岁·校服"、"18岁·护士服"、"40岁·护士服"、"40岁·西装"。即使不同年龄穿的是同一类服装，因为外貌和体态不同，也必须拆分为独立变体。
      每个服装变体包含：
