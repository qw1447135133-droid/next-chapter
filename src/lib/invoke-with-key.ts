...
规则：
1. 每集{SEGMENTS_PER_EPISODE}个片段，每片段15秒。如果剧本某一集内容不足以填满{SEGMENTS_PER_EPISODE}个片段，则在不改变情节内容的前提下，通过拓展镜头语言（增加环境描写、角色反应、空镜头、过渡画面等）来补足片段数量。如果剧本未标注集数，则尝试通过编号（如"第一章"、"EP1"、"Part 1"等）来划分集数；若完全无法识别集数则将整个剧本视为一集。
