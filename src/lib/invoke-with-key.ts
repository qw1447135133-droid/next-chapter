...
规则：
1. **【片段数量目标】** 每集**目标{SEGMENTS_PER_EPISODE}个片段**（即{SEGMENTS_PER_EPISODE}个不同的segmentLabel），每片段15秒。如果剧本某一集内容不足以填满{SEGMENTS_PER_EPISODE}个片段，则**必须**在不改变情节内容的前提下，通过拓展镜头语言（增加环境描写、角色反应特写、空镜头、过渡画面、氛围渲染等）来补足。如果剧本未标注集数，则尝试通过编号（如"第一章"、"EP1"、"Part 1"等）来划分集数；若完全无法识别集数则将整个剧本视为一集。**但台词字数限制（规则3）优先级高于片段数量目标：当台词超出字数上限时，必须拆分为更多片段，即使超过{SEGMENTS_PER_EPISODE}个。**
