...
export function buildCompliancePrompt(
  ...
  const market = setup.targetMarket || "cn";
  const isChinese = market === "cn" || market === "jp" || market === "kr";
...
检查每集对话是否符合以下标准：
- **每集台词总词数**：150-180个词（英文单词计数）
- **连续对白长度**：4-5个镜头的连续对白在20个词以内

检测方法：
1. 识别对话：找出所有角色说话的内容（通常用引号、冒号后的内容等）
2. 统计词数：计算每集所有对话的总词数（英文按单词计，中文按字计）
3. 检查连续对白：找出4-5个连续镜头的对白，检查是否超过20个词
4. 标记问题：超出限制的对话用 ⚠️ 标记
...
如果检测到对话超出限制，请提供调优后的版本：
1. **保持剧情不变**：调优后的对话必须传达相同的信息和情感
2. **精简表达**：去除冗余词语，保留核心信息
3. **拆分长对白**：将超长对白拆分到更多镜头中
4. **控制总量**：如果总词数超标，需要删减非必要对话

调优示例：
- 原对白（30词）："I really don't understand why you always treat me like this, every time I say something you never listen, I'm just so tired of all this"
- 调优后（12词）："You never listen to me. I'm exhausted."

注意：对于英文市场，按单词计数；对于中文市场，按字数计数（每集约150-180字）。`;
...
以下内容**永远不要标记**，除非触发字数限制：

1. **Dialogue (台词)** - 角色说的话，通常在引号内
   - 例如："我爱你"、"我杀了你"
   - 即使包含敏感词汇也不要标记
...
### 二、Scene Descriptions (场景描写)
- 能在画面中呈现的冲突场景
- 能在画面中呈现的亲密场景
- 未成年人参与的敏感场景

### 三、Dialogue Word Count (仅当超标时)
${dialogueRules}
...
export function buildStructureTransformPrompt(
  ...
  // 支持多风格融合（逗号分隔）
  const styles = frameworkStyle ? frameworkStyle.split(",").filter(Boolean) : [];
...
5. **逻辑自洽**：无论融合多么大胆，内部逻辑必须自洽` : "";
  ...
  // 不选择风格时的提示（保持原文风格，但名称必须不同）
  const noStyleGuide = !hasStyle ? `
## ⚠️ 同风格改编模式（未选择风格方向）
你将保持与原文相同的风格类型进行改编，核心要求是**创造一个查重率极低的新剧本**：

### 必须遵守的规则
1. **人物名称全部更换**：
   - 绝不能使用原文中的任何人物名、角色名、昵称、绰号
   - 新名字风格要与原文保持一致，但必须是全新的名字
   - 列出原文人物名 → 新人物名对照表

2. **地点名称全部更换**：
   - 绝不能使用原文中的任何地名、场所名、建筑名
   - 新地名风格要与原文保持一致，但必须是全新的地名
   - 列出原文地名 → 新地名对照表
...
4. **组织/势力名称更换**：
   - 原文中的门派、公司、组织、势力名称必须更换
   - 列出原文组织 → 新组织对照表

5. **核心情节保留**：
   - 情节骨架、冲突结构、人物关系拓扑必须保持
   - 但具体场景细节、对话内容需要重新创作

### 查重率要求
- 人物名、地名、道具名查重率：0%（完全不重复）
- 情节结构相似度：保持 70-80%（核心冲突一致）
- 场景细节相似度：低于 30%（重新描写）` : "";

  const styleSection = hasStyle ? `转换为「${styleText}」风格` : "保持原文风格进行低查重改编";

  return `你是一位专业的微短剧改编编剧，擅长将不同风格的故事进行框架转换${isMultiStyle ? "，尤其精通多风格融合创作" : ""}${!hasStyle ? "，尤其擅长在保持原文风格的同时创造全新的剧本内容" : ""}。

${getFullMarketDirective(setup)}
${fusionGuide}
${noStyleGuide}
## 你的任务
将以下参考剧本的叙事结构${styleSection}。

## 转换原则
1. **保留核心情节骨架**：主要矛盾冲突、人物关系拓扑、关键转折点必须保留
${hasStyle ? `2. **风格全面置换**：世界观、时代背景、社会体系、权力结构、文化元素全部替换为「${styleText}」设定
3. **等价替换法则**：
   - 原文中的社会阶层 → ${styleText}对应的等级体系
   - 原文中的权力机制 → ${styleText}对应的权力形式
   - 原文中的情感表达 → ${styleText}对应的情感方式
4. **强化风格特色**：加入${styleText}风格特有的元素、术语、场景设定` : `2. **名称全面更换**：所有人物名、地名、道具名、组织名必须更换为全新名称
3. **风格保持一致**：新名称的风格要与原文风格保持协调
4. **细节重新创作**：场景描写、对话内容需要重新撰写，降低查重率`}
...
1. **剧名备选**（3个），每个附一句话说明
${!hasStyle ? `2. **名称对照表**（必须包含）：
   - 人物名：原文 → 新名
   - 地名：原文 → 新名
   - 道具/组织：原文 → 新名
3. **时空背景**：保持与原文相同的时代、地点类型，但使用全新地名` : `2. **时空背景**：转换后的时代、地点、社会环境、体系设定${isMultiStyle ? `\n   - 特别说明「${styles[0]}」与「${styles[1]}」如何融合` : ""}`}
4. **一句话故事线** + **核心冲突**
5. **情节对照表**：原文核心情节 → 转换后对应情节（逐条对照）
6. **三幕结构拆解**：
   - 第一幕（建置）：集数范围、核心事件
   - 第二幕（对抗）：集数范围、冲突升级
   - 第三幕（高潮/结局）：集数范围、终极对决
${hasStyle ? `7. **${styleText}特色元素清单**：本风格必须包含的标志性场景/设定/术语${isMultiStyle ? `\n   - 「${styles[0]}」特有元素\n   - 「${styles[1]}」特有元素\n   - 融合创新元素` : ""}` : "7. **查重率说明**：说明如何确保与原文查重率低"}
8. **付费卡点规划**：具体集数 + 卡点类型
9. **结局设计**
