...
检查每集对话是否符合以下标准：
- **每集对话总字数**：不超过100字${isChinese ? "（中文）" : "（或对应的英文）"}
- **单句对话长度**：不超过35字${isChinese ? "（中文）" : "（或对应的英文）"}

检测方法：
1. 识别对话：找出所有角色说话的内容（通常用引号、冒号后的内容等）
2. 统计字数：计算每集所有对话的总字数，以及每句对话的字数
3. 标记问题：超出限制的对话用 ⚠️ 标记
...
3. **拆分长句**：将超长对话拆分成多个短句
4. **控制总量**：如果总对话字数超标，需要删减非必要对话

调优示例：
- 原句：「我真的不明白为什么你总是这样对我，每次我说什么你都不听，我真的好累好累啊」（45字）
- 调优：「你从不听我说话，我太累了」（12字）`;
