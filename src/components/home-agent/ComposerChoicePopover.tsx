        ...
        >
          <div className="flex justify-start">
            <div
              ...
              className={`w-full ${compactChoiceMode ? "max-w-[680px]" : "max-w-[560px]"} rounded-[20px] p-2.5 sm:p-3 ${
                dark
                  ? "bg-[linear-gradient(180deg,rgba(28,29,33,0.88),rgba(22,23,26,0.94))] shadow-[0_14px_34px_rgba(0,0,0,0.16)] backdrop-blur-xl"
                  : "bg-white/96 shadow-[0_14px_30px_rgba(148,163,184,0.14)]"
              }`}
            >
              <div className={`flex items-start gap-3 ${compactChoiceMode ? "mb-2" : "mb-2.5"}`}>
                <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-[14px] bg-[#0f62fe] text-white">
                  <Sparkles className="h-3.5 w-3.5" />
                </div>
                <div className="min-w-0 flex-1">
                  <div className="flex items-center justify-between gap-3">
                    <div className={`text-[13px] font-semibold ${dark ? "text-slate-100" : "text-slate-900"}`}>
                      {question.title}
                    </div>
                    {question.totalSteps > 1 ? (
                      <div
                        className={`rounded-full px-2 py-1 text-[10px] ${
                          dark
                            ? "border border-white/8 bg-white/6 text-slate-400"
                  ...
                  {question.description ? (
                    <div className={`mt-1 text-[12px] leading-5 ${dark ? "text-slate-400" : "text-slate-600"}`}>
                      {question.description}
                    </div>
                  ) : null}
                  <div className={`mt-1.5 text-[11px] ${dark ? "text-slate-500" : "text-slate-500"}`}>
                    {question.multiSelect
                      ? "可多选。先点选建议，再补充输入。"
                      : question.submissionMode === "confirm"
                        ? "先选一个方向，再确认继续。"
                  ...
                  {question.options.map((option) => (
                    <button
                      ...
                      className={`inline-flex min-h-10 items-center rounded-full border px-3.5 py-2 text-left text-[12.5px] font-medium transition-colors ${
                        option.selected
                          ? "border-[#2a73ff] bg-[#0f62fe]/18 text-white"
                          : dark
                            ? "border-white/[0.08] bg-white/[0.045] text-slate-200 hover:bg-white/[0.08]"
                ...
                <div className="space-y-1.5">
                  {question.options.map((option) => (
                    <button
                      ...
                      type="button"
                      onClick={() => onSelect(option.value, option.label)}
                      className={`group w-full rounded-[16px] px-3.5 py-2.5 text-left transition-colors ${
                        option.selected
                          ? "bg-[#0f62fe]/18 text-white"
                          : dark
                      ...
                      {option.rationale ? (
                        <div
                          className={`mt-1 text-[10.5px] leading-4.5 ${
                            option.selected ? "text-white/80" : dark ? "text-slate-500" : "text-slate-500"
                          }`}
                        >
                          {option.rationale}
                        </div>
              ...
              {(question.submissionMode === "confirm" || question.multiSelect) && onConfirm ? (
                <div className="mt-3 flex items-center justify-between gap-3">
                  ...
                  <Button
                    type="button"
                    size="sm"
                    className="rounded-full bg-[#0f62fe] px-4 text-white hover:bg-[#1b6fff]"
                    onClick={onConfirm}
                    disabled={!canConfirm}
                  >
                    继续
                ...
                <div className={`mt-2.5 text-[11px] ${dark ? "text-slate-500" : "text-slate-500"}`}>
