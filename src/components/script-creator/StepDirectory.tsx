...
import { ArrowRight, RefreshCw, Pencil, Eye, Loader2 } from "lucide-react";
...
const StepDirectory = ({
  ...
  const hookDistribution = directory.reduce<Record<string, number>>(
    (acc, ep) => {
  ...
  const segments = [
    {
      label: "起势段",
      range: [1, Math.round(total * 0.15)],
      color: "bg-blue-500",
    },
    {
      label: "攀升段",
      range: [Math.round(total * 0.15) + 1, Math.round(total * 0.45)],
      color: "bg-green-500",
    },
    {
      label: "风暴段",
      range: [Math.round(total * 0.45) + 1, Math.round(total * 0.8)],
      color: "bg-amber-500",
    },
    {
      label: "决战段",
      range: [Math.round(total * 0.8) + 1, total],
      color: "bg-red-500",
    },
  ];
  ...
  const segmentCounts = segments.map((seg) => ({
    ...
    count: directory.filter(
      (ep) => ep.number >= seg.range[0] && ep.number <= seg.range[1],
    ).length,
            ...
            {directoryRaw && !isGenerating && (
              <>
                ...
                <Button
                  ...
                  size="sm"
                  onClick={() =>
                    editing ? handleEditSave() : setEditing(true)
                  }
                  className="gap-1.5"
                >
                  {editing ? (
            ...
            <div className="space-y-4">
              ...
              {directory.length > 0 && (
                <div className="space-y-3 border rounded-lg p-4 bg-muted/20">
                  ...
                  {/* Emotional Waveform - K线风格 */}
                  {directory.length > 0 && (
                    <div className="border-t pt-3">
                      <div className="flex items-center justify-between mb-2">
                        <p className="text-xs font-semibold">🌊 情绪波形图</p>
                      ...
                      <div className="flex gap-2">
                        {/* Y-axis labels */}
                        <div className="flex flex-col justify-between text-[10px] text-muted-foreground py-0.5 shrink-0 w-5">
                          <span>5</span>
                          <span>3</span>
                          <span>1</span>
                        </div>
                        {/* Chart area */}
                        <div className="flex-1 min-w-0 relative aspect-[8/1]">
                          <svg
                          ...
                          >
                            ...
                            {(() => {
                              const getLevel = (ep: EpisodeEntry) => {
                                if (ep.emotionLevel != null)
                                  return ep.emotionLevel;
                                if (ep.isClimax) return 5;
                                if (ep.isPaywall) return 4;
                                if (ep.isKey) return 3;
                                return 2;
                              };
                              const levels = directory.map(getLevel);
                              const n = levels.length;
                              if (n === 0) return null;
                              const padding = n > 30 ? 1 : 2;
                              const candleWidth = (100 - padding * 2) / n;
                              ...
                              return (
                                <>
                                  ...
                                  />
                                  {directory.map((ep, i) => {
                                    const level = getLevel(ep);
                                    const x =
                                      padding + i * candleWidth + gap / 2;
                                    const y = toY(level);
                                    const height = 9 - y; /* 1→0, 3→4, 5→8 */
                                    let fill = "hsl(var(--primary))";
                                    if (ep.isClimax) fill = "#f59e0b";
                                    else if (ep.isPaywall) fill = "#eab308";
                                    else if (level > 3) fill = "#ef4444";
                                    else if (level > 2) fill = "#f97316";
                                    return (
                                      <g key={ep.number}>
                                        <rect
                                          x={x}
                      ...
                      <div className="flex justify-between text-[10px] text-muted-foreground mt-1 ml-7">
                        <span>第1集</span>
                  ...
                  <div className="flex gap-4 text-xs text-muted-foreground border-t pt-2 flex-wrap">
                    <span>
                      🔥 关键集 {keyCount} (
                      {directory.length > 0
                        ? Math.round((keyCount / directory.length) * 100)
                        : 0}
                      %，目标25-35%)
                    </span>
                    <span>
                      ⚡ 高潮卡点 {climaxCount} (
                      {directory.length > 0
                        ? Math.round((climaxCount / directory.length) * 100)
                        : 0}
                      %，目标10-15%)
                    </span>
                    <span>
                      💰 付费卡点 {paywallCount} (
                      {directory.length > 0
                        ? Math.round((paywallCount / directory.length) * 100)
                        : 0}
                      %，目标10-15%)
                    </span>
                  </div>
              ...
              <div className="max-h-[500px] overflow-auto space-y-1">
                {directory.map((ep) => (
                  <div key={ep.number}>
                    ...
                    >
                      ...
                      <span
                        className={`w-2 h-2 rounded-full mt-1.5 shrink-0 ${getSegmentForEp(ep.number).color}`}
                      />
                ...
                {directory.length === 0 &&
                  ...
                  (showTranslation && hasTranslation(directoryRaw) ? (
                    <InterleavedText
                      text={directoryRaw}
                      translatedLines={getTranslation(directoryRaw)!}
                    />
                  ) : (
